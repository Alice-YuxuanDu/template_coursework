...
plot(x = penguins$bill_length_mm, 
     y = penguins$bill_depth_mm)
```
We can see from the plot that there is a general positive association between bill length and bill depth.
...
```{r}
mean(penguins$body_mass_g, na.rm = TRUE)
median(penguins$body_mass_g, na.rm = TRUE) #na.rm = TRUE means to ignore missing values
```
...
```
We can see that for each species there is a different average weight, where Gentoo penguins are the heaviest, and Adelie and Chinstrap have similar average weights.
...
```
The linear model shows that the average weight of an Adelie penguin is around 3700g, while Chinstrap and Gentoo penguins are on average around 32g and 1375g heavier respectively. The standard errors show us the uncertainty in these estimates.
...
plot(
  ...
  xlab = "Length (mm)",
  ylab = "Depth (mm)", 
  pch = 16) # pch is plotting character，是一个整数，表示点的形状
```
